...
function parseBibEntries(source: string): ParsedBibEntry[] {
  const content = source.replace(/^\s*%.*$/gm, '');
  ...
  while (cursor < content.length) {
    ...
    for (; index < content.length && depth > 0; index += 1) {
      ...
      if (char === '"' && content[index - 1] !== '\\') quoted = !quoted;
      if (!quoted) {
        if (char === '{') depth += 1;
        if (char === '}') depth -= 1;
      }
    ...
    const body = content.slice(header.lastIndex, Math.max(header.lastIndex, index - 1));
    const entryTags: Record<string, string> = {};
    splitTopLevel(body).forEach((field) => {
      const equals = field.indexOf('=');
      if (equals < 0) return;
      const key = field.slice(0, equals).trim().toLowerCase();
      const fieldValue = unwrapBibValue(field.slice(equals + 1));
      if (key) entryTags[key] = fieldValue;
    });
...
export function parseBibTeX(bibtexContent: string, locale?: string): Publication[] {
  ...
  const entries = parseBibEntries(bibtexContent);
